...
# Write proxy URLs (always if present, supports mixed mode)
cli_proxy = providers.get('cli_proxy', {})
...
| Agent | Role | Provider | Connection | Model | Status |
|-------|------|----------|------------|-------|--------|
EOF
...
# Determine which agents use proxies vs direct API
proxy_providers = set()
cli_proxy = config.get('providers', {}).get('cli_proxy', {})
for p in cli_proxy.get('enabled', []):
    proxy_providers.add(p)

with open(os.path.join(vault_path, 'AGENT_ROSTER.md'), 'a') as f:
    for agent_id in enabled:
        ...
        if provider in ('anthropic', 'claude') and 'claude' in proxy_providers:
            connection = 'proxy'
        elif provider in ('google', 'gemini') and 'gemini' in proxy_providers:
            connection = 'proxy'
        else:
            connection = provider_mode

        f.write(f"| {name} | {agent_id} | {provider} | {connection} | {model} | active |\n")
PYEOF
